...
cityBtn.on("click", function() {
  generateWeather(storedCities);
});

//display weather for a new city--------------------------------//
searchBtn.on("click", function () {
  generateWeather(cityQuery.val());
});
...
function generateWeather(city){
  ...
  currentCityDiv.html(" ");
  forecastDiv.html(" ");
  var queryURL = "https://api.openweathermap.org/data/2.5/weather?q=" + city + owKey;
  $.ajax({
    url: queryURL,
    method: "GET",
    ...
    $.ajax({
      ...
      $.ajax({
        ...
        forecastArray = [];
        forecastArray.push(response.list[10], response.list[18], response.list[26], response.list[34], response.list[39]);
        ...
        for (let i = 0; i < forecastArray.length; i++) {
          ...
          //date element
          var todaysDate = forecastArray[i].dt_txt.slice(5,10);
          var forecastDate = $("<p>");
